...
public class mainLogic {
	...
	public Tree<Entry<String, PositionList<String>>> createTree(String path) {
		...
		try {
			Pair<String, PositionList<String>> rootSign = new Pair<>(directorio.getRutaCompleta(), new ListaDE<>());
			tree.createRoot(rootSign);

			subTree(tree, tree.root(), directorio);

			for (Position<Entry<String, PositionList<String>>> hijosRaiz : tree.children(tree.root())) {
				tree.root().element().getValue().addFirst(hijosRaiz.element().getKey());
			}
	...
	private void subTree(Tree<Entry<String, PositionList<String>>> tree,Position<Entry<String, PositionList<String>>> p, Directorio d) {
		try {
			for (String archive : d.archivosDelDirectorio()) {
				tree.addLastChild(p, new Pair<>(archive, null));
			}

			for (String dir : d.subdirectoriosDelDirectorio()) {
				Position<Entry<String, PositionList<String>>> pos = tree.addFirstChild(p, new Pair<>(dir, new ListaDE<>()));
				subTree(tree, pos, new Directorio(dir)); // Crear el Directorio una sola vez
			}
	 ...
	 */
	private TNodo<Entry<String, PositionList<String>>> checkPosition(Position<Entry<String, PositionList<String>>> location) throws InvalidPositionException {
